...
public class FileChooserPanel extends JPanel {
	...
	private class FileListModel implements ListModel, Comparator<File> {
		...
		private void insertTopFiles(final LinkedList<File> directoryQueue, final FileFilter fileFilter,
				final File[] files) {
...
//	private final ImagePreviewListener m_previewListener;
	...
	public FileChooserPanel(final FileFilter fileFilter) {
		...
		m_addButton = new JButton("Add Selected", new ImageIcon(getClass().getResource(BUTTON_OK)));
		m_addAllButton = new JButton("Add all visible files", new ImageIcon(getClass().getResource(EDIT_ADD)));
		m_remButton = new JButton("Remove selected", new ImageIcon(getClass().getResource(EDIT_DELETE)));
		m_remAllButton = new JButton("Remove all Files", new ImageIcon(getClass().getResource(EDIT_REMOVE)));
		...
		final JMenuItem add = new JMenuItem("Add Selected File",
				new ImageIcon(getClass().getResource(BUTTON_OK)));
		...
		final JMenuItem addAll = new JMenuItem("Add all Visible Files",
				new ImageIcon(getClass().getResource(EDIT_ADD)));
		...
		final JMenuItem clearSelection = new JMenuItem("Remove All",
				new ImageIcon(getClass().getResource(EDIT_REMOVE)));
		...
		final JMenuItem remove = new JMenuItem("Remove Selected",
				new ImageIcon(getClass().getResource(EDIT_DELETE)));
			...
			public void mouseClicked(final MouseEvent arg0) {
				...
				if (arg0.isControlDown()) {
					...
					final File dir = new File(
							m_selectedFileListModel.getAbsolutePathAt(m_selectedFileList.getSelectedIndices()[0]));
...
//		}
	...
	public void onAdd() {
		...
		if (m_fileChooser.getSelectedFiles().length == 0) {
			...
			JOptionPane.showMessageDialog(this, "No files selected. Please select at least one file or directory.",
	...
	private void onRemove() {
		if (m_selectedFileListModel.getSize() == 0) {
			...
			JOptionPane.showMessageDialog(this, "No files in list to Remove", "Error", JOptionPane.ERROR_MESSAGE);
